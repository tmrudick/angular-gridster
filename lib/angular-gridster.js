...
angular.module('gridster', [])
    ...
    return {
      ...
      controller: function($scope) {
        ...
        this.options = function(arg) {
          return options = $parse(arg)() || options;
        }

        this.editable = function(arg) {
          return editable = $parse(arg)() || editable;
        }

        this.changedFn = function(arg) {
          return changedFn = $parse(arg) || changedFn;
        }
  ...
  .directive('ngGridsterWidget', ['$animate', '$timeout', function($animate, $timeout) {
    return {
      restrict: 'A',
      transclude: 'element',
      require: '^ngGridster',
      ...
      link: function(scope, element, attr, controller, transclude) {
        var expression = attr.ngGridsterWidget, // Get the repeat expression
            gridId = attr.gridId,
        ...
        var watcher = scope.$watchCollection(collectionExp, function(collection) {
          ...
          $timeout(function() {
            ...
            // Send resize events to all widgets
            element.siblings('li.gridster-widget').each(function(index, el) {
              var $widget = $(el);
              $widget.scope().$broadcast('widget-resized', {
            ...
            // Set the gridster object on the gridster controller so we can later use the gridster api
