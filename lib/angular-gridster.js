    ...
    return {
      ...
      link: function(scope, element, attr, controller) {
        scope.$on('gridster-repeat-complete', function(e) {
          ...
          // Parse options
          var options = $parse(attr.ngGridster)() || {};

          // Deal with the changed function
          if (attr.gridsterChanged) {
        ...
        var watcher = scope.$watchCollection(collectionExp, function(collection) {
          ...
          for (; index < length; index++) {
            ...
            transclude(childScope, function(clone) {
              ...
              var layout = $parse(attr.layout)(childScope);
              if (layout) {
                ...
                clone.attr('data-row', layout.row);
                clone.attr('data-col', layout.col);

                // Default to 1x1 is no size if specified
                clone.attr('data-sizex', layout.size_x || 1);
                clone.attr('data-sizey', layout.size_y || 1);
              }
